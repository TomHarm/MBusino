/*
# MBusino: M-Bus--> MQTT-Gateway
Ethernet - Testcode, not released
...
void setup() {
  ...
  MbusSerial.begin(MBUS_BAUD_RATE, SERIAL_8E1,20,21);
  ...
  if (!client.connected() && ((millis() - timerReconnect) > 5000)) {  
    ...
    if((millis()-timerDebug) >10000){
      ...
      if(usedMQTTconection == 1){
        client.publish(String(String(userData.mbusinoName) + "/settings/MQTTconnectedOver").c_str(), "ethernet");
      }
      if(usedMQTTconection == 2){
        client.publish(String(String(userData.mbusinoName) + "/settings/MQTTconnectedOver").c_str(), "WiFi");
      }
      long rssi = WiFi.RSSI();
